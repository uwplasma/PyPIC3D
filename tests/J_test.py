...
class TestJFunctions(unittest.TestCase):
    def setUp(self):
        ...
        self.grid = jnp.arange(-self.x_wind/2, self.x_wind/2, self.dx), jnp.arange(-self.y_wind/2, self.y_wind/2, self.dy), jnp.arange(-self.z_wind/2, self.z_wind/2, self.dz)
    ...
    def test_J_first_order_weighting(self):
        Jx, Jy, Jz = J_first_order_weighting(self.q, self.x, self.y, self.z, self.vx, self.vy, self.vz, self.num_J, self.dx, self.dy, self.dz, self.x_wind, self.y_wind, self.z_wind, self.grid)
    ...
    def test_J_second_order_weighting(self):
        ...
        Jx, Jy, Jz = J_second_order_weighting(self.q, float(self.x[0]), float(self.y[0]), float(self.z[0]), float(self.vx[0]), float(self.vy[0]), float(self.vz[0]), self.J, self.dx, self.dy, self.dz, self.x_wind, self.y_wind, self.z_wind, self.grid)
        self.assertEqual(Jx.shape, (10,10,10))
        self.assertEqual(Jy.shape, (10,10,10))
        self.assertEqual(Jz.shape, (10,10,10))
        # make sure the shapes are correct
